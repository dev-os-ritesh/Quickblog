...
const CommentTableItem = ({comment,fetchComments}) => {
    ...
    const BlogDate = new Date(createdAt);
     ...
     const approveComment = async () => {
         try {
             const {data} = await axios.post('/api/admin/approveComment', {id: comment._id});
             if(data.success){
      ...
      const deleteComment = async ()=>{
        try {
            ...
            if(!confirm) return;
            const {data} = await axios.post('/api/admin/deleteComment', {id: comment._id});
            if(data.success){
                toast.success(data.message);
                await fetchComments();
            }else{
  ...
  return (
    <tr className='order-y border-gray-300 '>
        <td className='px-6 py-4'>
            <b className='font-medium text-gray'>Blog</b> : {blog.title}
            <br />
            <br />
